...
namespace cms::alpakatools {
  ...
  make_host_buffer(TQueue const& queue, Extent extent) {
    if constexpr (allocator_policy<alpaka::Dev<TQueue>> == AllocatorPolicy::Caching) {
      ...
      return alpaka::allocMappedBuf<alpaka::Platform<alpaka::Dev<TQueue>>, std::remove_extent_t<T>, Idx>(host, platform, Vec1D{extent});
    }
  }
  ...
  make_host_buffer(TQueue const& queue) {
    if constexpr (allocator_policy<alpaka::Dev<TQueue>> == AllocatorPolicy::Caching) {
      ...
      return alpaka::allocMappedBuf<alpaka::Platform<alpaka::Dev<TQueue>>, std::remove_extent_t<T>, Idx>(host, platform, Vec1D{std::extent_v<T>});
    }
  }
