...
namespace BrokenLine {
  ...
  */
  inline double MultScatt(const double& length, const double B, const double R, int Layer, double slope) {
    ...
    return fact / (pt2 * (1. + Rfit::sqr(slope))) * (::hipsycl::sycl::detail::__hipsycl_abs(length) * XXI_0) *
           Rfit::sqr(1. + 0.038 * log(::hipsycl::sycl::detail::__hipsycl_abs(length) * XXI_0));
  }
  ...
  */
  ...
  inline void BL_Fast_fit(const M3xN& hits, V4& result) {
    ...
    result(2) = sqrt(a.squaredNorm() * b.squaredNorm() * c.squaredNorm()) / (2. * ::hipsycl::sycl::detail::__hipsycl_abs(Rfit::cross2D(b, a)));
  ...
  */
  ...
  inline Rfit::helix_fit BL_Helix_fit(const Rfit::Matrix3xNd<N>& hits,
    ...
    jacobian << 1., 0, 0, 0, 1., 0, 0, 0, -::hipsycl::sycl::detail::__hipsycl_abs(circle.par(2)) * B / (Rfit::sqr(circle.par(2)) * circle.par(2));
    circle.par(2) = B / ::hipsycl::sycl::detail::__hipsycl_abs(circle.par(2));
    circle.cov = jacobian * circle.cov * jacobian.transpose();
