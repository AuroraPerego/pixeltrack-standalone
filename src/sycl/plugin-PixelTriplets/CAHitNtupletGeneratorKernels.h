...
public:
  ...
  using HitsView = TrackingRecHit2DSOAView;
  using HitsOnGPU = TrackingRecHit2DSOAView;
  using HitsOnCPU = TrackingRecHit2DHeterogeneous<Traits>; //when FIXME_ the other one remove also this and its template
  ...
  void launchKernels(HitsOnCPU const& hh, TkSoA* tuples_d, sycl::queue stream);

  void classifyTuples(HitsOnCPU const& hh, TkSoA* tuples_d, sycl::queue stream);

  void fillHitDetIndices(HitsView const* hv, TkSoA* tuples_d, sycl::queue stream);
