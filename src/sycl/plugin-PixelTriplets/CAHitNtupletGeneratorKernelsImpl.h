...
// assert(nCells);
  ...
  for (int idx = first, nt = (*nCells); idx < nt; idx += item.get_group_range(0) * item.get_local_range().get(0)) {
    ...
    auto score = [&](auto it) {
      return ::hipsycl::sycl::detail::__hipsycl_abs(tracks->tip(it));  // tip
...
void kernel_classifyTracks(HitContainer const *__restrict__ tuples,
  ...
  // assert(quality[it] == trackQuality::bad);
    ...
    bool isOk = (::hipsycl::sycl::detail::__hipsycl_abs(tracks->tip(it)) < region.maxTip) and (tracks->pt(it) > region.minPt) and
                (::hipsycl::sycl::detail::__hipsycl_abs(tracks->zip(it)) < region.maxZip);
...
void kernel_tripletCleaner(
  ...
  for (int idx = first, ntot = hh.nHits(); idx < ntot; idx += item.get_group_range(0) * item.get_local_range().get(0)) {
    ...
    for (auto ip = hitToTuple.begin(idx); ip != hitToTuple.end(idx); ++ip) {
      auto const it = *ip;
      if (quality[it] != bad && ::hipsycl::sycl::detail::__hipsycl_abs(tracks.tip(it)) < mc) {
        mc = ::hipsycl::sycl::detail::__hipsycl_abs(tracks.tip(it));
        im = it;
      }
    }
    // mark duplicates
