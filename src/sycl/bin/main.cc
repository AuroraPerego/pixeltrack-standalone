...
namespace {
  void print_help(std::string const& name) {
    std::cout
        ...
        << " --device            Specifies the device which should run the code (default all, options: "
           "<backend>:<devices> "
           "with <backend>={ * | level_zero | opencl | cuda | hip | esimd_emulator } and <devices>={ * | cpu | gpu | "
           "fpga | <num> | <num>.<num> | <num>.* | *.* | <num>.<num>.<num> | <num>.<num>.* | <num>.*.* | *.*.*  } "
